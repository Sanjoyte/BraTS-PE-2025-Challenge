function nsct_hf(baseDir, nsct_tbx_dir, casePattern)
%NSCT_HF  NSCT high frequency subband extraction (4 subbands per modality).
%
%   nsct_hf(BASEDIR, NSCT_TBX_DIR, CASEPATTERN)
%
%   BASEDIR       directory holding one sub-folder per case
%   NSCT_TBX_DIR  path to the bundled nsct_toolbox
%   CASEPATTERN   glob for the case folders, e.g. 'BraTS-PED-*'
%
%   Paths used to be hardcoded to the original author's machine and to the
%   BraTS-2020 folder naming; they are arguments now so the same file works on
%   any dataset. The decomposition itself is unchanged.

if nargin < 2 || isempty(nsct_tbx_dir)
    nsct_tbx_dir = fullfile(fileparts(mfilename('fullpath')), 'nsct_toolbox');
end
if nargin < 3 || isempty(casePattern)
    casePattern = 'BraTS-PED-*';
end
...
patientDirs = dir(fullfile(baseDir, casePattern));

% 转换为 cell 数组便于 parfor
% Convert to cell array for parfor
patientFolderNames = {patientDirs([patientDirs.isdir]).name};
patientFolderNames = patientFolderNames(~ismember(patientFolderNames, {'.', '..'}));
